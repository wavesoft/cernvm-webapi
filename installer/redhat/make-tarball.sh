...
prepare:
	mkdir -p \$(BUILDDIR)
	cd \$(BUILDDIR); \$(CMAKE_BIN) -DSYSTEM_OPENSSL=ON -DCMAKE_BUILD_TYPE=Release -DCRASH_REPORTING=ON -DTARGET_ARCH=$(uname -p) -DCMAKE_INSTALL_PREFIX=\$(DESTDIR) -DINSTALL_ETC=\$(DESTDIR)/etc ../\$(SOURCE_DIR)
